...
def build(
    ...
    if with_charts:
        charts.sweep()
        payload["charts"] = [
            # The pivot the detector found, drawn on the chart. Without it the
            # reader has a price history; with it they can see the distance the
            # payload is describing. None when there is no setup, which draws
            # nothing rather than a line at zero.
            charts.candles(ticker, window, lookback_days, pivot=setup.pivot),
