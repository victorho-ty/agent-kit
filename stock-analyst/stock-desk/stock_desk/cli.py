...
from . import charts, clock, db, events, macro, markets, news, portfolio, report, runs
...
def cmd_news_poll(args) -> int:
    ...
    with db.connect() as conn:
        ...
        # The window is the gap since news last read cleanly, not a fixed
        # lookback: Alpha Vantage honours time_from, so this neither re-reads
        # yesterday nor loses a day after an outage.
        since = db.last_success(conn, "news_poll")
        result = news.poll(
            conn, entries, now, av_budget=args.av_budget, since=since, held=held
        )
...
def cmd_run(args) -> int:
    """One scheduled profile, end to end. The only thing cron ever invokes."""
    now = clock.now()
    config = watchlist_config.load()
    spec = runs.profile(args.profile)
    with db.connect() as conn:
        run_id = db.start_run(conn, f"run:{spec.name}", now)
        payload = runs.execute(conn, config, spec, now, commit=args.commit)
        status = "error" if payload["run"]["degraded"] and not payload.get("ok") else (
            "partial" if payload["run"]["degraded"] else "ok"
        )
        db.finish_run(
            conn, run_id, clock.now(), status, json.dumps(payload["run"]["failures"])
        )
        return _emit(payload)


def cmd_macro(args) -> int:
    """Where rates are right now. Reads; stamps nothing."""
    config = watchlist_config.load()
    with db.connect() as conn:
        series = macro.series_for(config.macro)
        if args.refresh:
            db.start_run(conn, "macro_refresh", clock.now())
            macro.refresh(conn, series, clock.now(), budget=args.budget)
        payload = macro.snapshot(conn, series)
        payload["ok"] = True
        return _emit(payload)


def cmd_sector(args) -> int:
    """Every sector measured over the default horizon. Reads; stamps nothing."""
    config = watchlist_config.load()
    with db.connect() as conn:
        views, standings = report.sector_section(conn, config)
        return _emit({"ok": True, "sectors": views, "standings": standings})
...
def build_parser() -> argparse.ArgumentParser:
    ...
    news_poll = news_parser.add_parser("poll", help="fetch feeds (silent; wakes nobody)")
    news_poll.add_argument(
        "--av-budget",
        type=int,
        default=0,
        help=(
            "how many Alpha Vantage calls this poll may spend (default 0 = Yahoo "
            "only). The free tier allows 25 a day across the whole profile, macro "
            "readings included."
        ),
    )
    news_poll.set_defaults(func=cmd_news_poll)
    ...
    run_parser = sub.add_parser(
        "run", help="one scheduled profile end to end -- what cron invokes"
    )
    run_parser.add_argument("profile", choices=sorted(runs.PROFILES))
    run_parser.add_argument(
        "--commit",
        action="store_true",
        help="stamp news, events and macro as reported. Run it, then send.",
    )
    run_parser.set_defaults(func=cmd_run)

    macro_parser = sub.add_parser("macro", help="where rates are (reads; stamps nothing)")
    macro_parser.add_argument("--refresh", action="store_true", help="fetch before reading")
    macro_parser.add_argument("--budget", type=int, default=3, help="max Alpha Vantage calls")
    macro_parser.set_defaults(func=cmd_macro)

    sector_parser = sub.add_parser("sector", help="sector standing (reads; stamps nothing)")
    ...
    runs_parser = sub.add_parser("runs", help="recent run health")
    runs_parser.add_argument("--limit", type=int, default=10)
    runs_parser.add_argument("--kind")
